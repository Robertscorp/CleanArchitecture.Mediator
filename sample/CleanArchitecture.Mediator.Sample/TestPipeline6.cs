...
internal class TestPipeline6
{
    ...
    private static readonly MethodInfo s_InvokeValidationPipeAsyncMethodInfo = typeof(TestPipeline5).GetMethod(nameof(InvokeValidationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo s_InvokeInteractorPipeAsyncMethodInfo = typeof(TestPipeline5).GetMethod(nameof(InvokeInteractorInvocationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;

    private static PipeHandleAsync<object> BuildPipeline(Type inputType, Type outputType)
    {
        var _Stages = new List<Stage>();

        if (outputType.IsAssignableTo(typeof(IAuthenticationFailureOutputPort)))
            _Stages.Add(new(GetStageDelegate(inputType, outputType, s_InvokeAuthenticationPipeAsyncMethodInfo)));
        ...
        if (inputType.IsAssignableTo(typeof(IInputPort<IAuthorisationPolicyFailureOutputPort<SampleInputPortValidationFailure>>))
            ...
            _Stages.Add(new(GetStageDelegate(inputType, outputType, s_InvokeAuthorisationPipeAsyncMethodInfo)));
        ...
        if (inputType.IsAssignableTo(typeof(IInputPort<IInputPortValidationFailureOutputPort<SampleInputPortValidationFailure>>))
            ...
            _Stages.Add(new(GetStageDelegate(inputType, outputType, s_InvokeInputPortValidationPipeAsyncMethodInfo)));

        _Stages.Add(new(GetStageDelegate(inputType, outputType, s_InvokeValidationPipeAsyncMethodInfo)));
        _Stages.Add(new(GetStageDelegate(inputType, outputType, s_InvokeInteractorPipeAsyncMethodInfo)));
        ...
        return new PipeHandleAsync<object>(async (input, output, factory, ct) =>
        {
            ...
            for (var _Index = 0; _Index < _StageArray.Length; _Index++)
                if (_Continuation is null)
                    _Continuation = await _StageArray[_Index].InvokeAsync(input, output, factory, ct);

                else
                    await _Continuation.HandleAsync(new(async () => _Continuation = await _StageArray[_Index].InvokeAsync(input, output, factory, ct)), ct);
        });
    }

    private static Func<object, object, ServiceFactory, CancellationToken, Task<ContinuationBehaviour>> GetStageDelegate(Type inputType, Type outputType, MethodInfo genericMethod)
    {
    ...
    private readonly struct Stage(Func<object, object, ServiceFactory, CancellationToken, Task<ContinuationBehaviour>> invokeAsync)
    {

        #region - - - - - - Properties - - - - - -

        public readonly Func<object, object, ServiceFactory, CancellationToken, Task<ContinuationBehaviour>> InvokeAsync = invokeAsync;
