...
internal partial class TestPipeline3
{

    private static readonly MethodInfo s_InvokeAuthenticationPipeAsyncMethodInfo = typeof(TestPipeline3).GetMethod(nameof(InvokeAuthenticationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo s_InvokeAuthorisationPipeAsyncMethodInfo = typeof(TestPipeline3).GetMethod(nameof(InvokeAuthorisationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo s_InvokeInputPortValidationPipeAsyncMethodInfo = typeof(TestPipeline3).GetMethod(nameof(InvokeInputPortValidationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo s_InvokeValidationPipeAsyncMethodInfo = typeof(TestPipeline3).GetMethod(nameof(InvokeValidationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
    private static readonly MethodInfo s_InvokeInteractorPipeAsyncMethodInfo = typeof(TestPipeline3).GetMethod(nameof(InvokeInteractorInvocationPipeAsync), BindingFlags.NonPublic | BindingFlags.Static)!;
        ...
        CancellationToken cancellationToken)
    {
        ...
        return ((Func<object, TOutputPort, ServiceFactory, CancellationToken, Task>)s_InvokeHandleCache
            .GetOrAdd(
                ...
                key =>
                {
                    var _AuthenticationPipeHandle = GetFunc<TOutputPort>(key, s_InvokeAuthenticationPipeAsyncMethodInfo);
                    ...
                    if (inputPort is IInputPort<IAuthorisationPolicyFailureOutputPort<SampleInputPortValidationFailure>> &&
                        ...
                        _AuthorisationPipeHandle = GetFunc<TOutputPort>(key, s_InvokeAuthorisationPipeAsyncMethodInfo);
                    ...
                    if (inputPort is IInputPort<IInputPortValidationFailureOutputPort<SampleInputPortValidationFailure>> &&
                        ...
                        _InputPortValidationPipeHandle = GetFunc<TOutputPort>(key, s_InvokeInputPortValidationPipeAsyncMethodInfo);

                    var _ValidationPipeHandle = GetFunc<TOutputPort>(key, s_InvokeValidationPipeAsyncMethodInfo);

                    var _InteractorPipeHandle = GetFunc<TOutputPort>(key, s_InvokeInteractorPipeAsyncMethodInfo);

                    return async (object inputPort, TOutputPort outputPort, ServiceFactory serviceFactory, CancellationToken cancellationToken) =>
                    {
                        // Authentication Pipe
