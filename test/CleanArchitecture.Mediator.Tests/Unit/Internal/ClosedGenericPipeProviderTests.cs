...
public class ClosedGenericPipeProviderTests
{
    ...
    public void Constructor_PipeTypeIsNotAnOpenGeneric_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(NonGenericPipe), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_PipeTypeDoesNotImplementIPipeInterface_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(DoesNotImplementGenericIPipe<>), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_PipeTypeImplementsIPipeInterfaceMultipleTimes_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(ImplementsGenericIPipeTwice<>), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_IPipeInterfaceIsImplementedWithClosedGenericInputPort_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(ClosedGenericInputPort<>), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_IPipeInterfaceIsImplementedWithClosedGenericOutputPort_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(ClosedGenericOutputPort<>), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_IncorrectNumberOfClosedGenericTypesSpecified_FailsToConstruct()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(ValidPipe<,,,,>), [])).Should().BeOfType<ArgumentException>();

    [Fact]
    public void Constructor_ValidSetup_ConstructsSuccessfully()
        => Record.Exception(() => new ClosedGenericPipeProvider(typeof(ValidPipe<,,,,>), [typeof(object), typeof(object), typeof(object)])).Should().BeNull();
    ...
    public void GetPipe_GenericConstraintsAreNotMet_NoPipeReturned()
        => ((IClosedGenericPipeProvider)new ClosedGenericPipeProvider(typeof(ConstrainedPipe<,>), []))
    ...
    public void GetPipe_InputAndOutputPortParametersAreBetweenGenericParameters_ResolvesGenericsProperlyAndGetsPipeFromServiceFactory()
        => ((IClosedGenericPipeProvider)new ClosedGenericPipeProvider(typeof(ValidPipe<,,,,>), [typeof(object), typeof(string), typeof(int)]))
